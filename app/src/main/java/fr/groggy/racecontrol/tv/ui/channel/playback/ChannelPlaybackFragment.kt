...
class ChannelPlaybackFragment : VideoSupportFragment(), Player.Listener {
    ...
    private val videoHostLayoutChangeListener = View.OnLayoutChangeListener { _, left, top, right, bottom, oldLeft, oldTop, oldRight, oldBottom ->
        ...
        Log.d(TAG, "Video host resized to ${newWidth}x${newHeight}; locking surface to viewport")
        lockPlaybackSurfaceToHostViewport("hostLayoutChange")
    }
    ...
    private fun setMainPlayerAudioDisabled(disabled: Boolean, reason: String) {
        // Only toggle audio disable — rebuilding via applyPlaybackVideoConstraints() would
        // reset max/min video size and bounce a manual quality pick back to Auto.
        trackSelector.setParameters(
            trackSelector.buildUponParameters()
                .setTrackTypeDisabled(C.TRACK_TYPE_AUDIO, disabled)
                .build()
        )
        Log.i(TAG, "Main player audio disabled=$disabled while preserving video quality constraints reason=$reason")
    }
            ...
            if (_player == null) {
                ...
                _player = ExoPlayer.Builder(requireContext(), renderersFactory)
                    ...
                    .build().also { p ->
                        ...
                        // Crop to fill the locked host viewport so ABR resolution
                        // changes never appear as zoom in/out on the SurfaceView.
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        view.post {
            lockPlaybackSurfaceToHostViewport("onViewCreated")
        }
    }
    ...
    private fun bindPlaybackVideoSurface(surfaceView: SurfaceView, source: String) {
        ...
        lockPlaybackSurfaceToHostViewport("bindPlaybackVideoSurface-$source")
    }
    ...
    override fun onVideoSizeChanged(videoSize: androidx.media3.common.VideoSize) {
        ...
        // Live ABR changes stream width/height frequently. Do NOT forward those
        // dimensions into VideoSupportFragment.onVideoSizeChanged() — Leanback
        // would resize the SurfaceView and look like random zoom in/out.
        // Keep the surface locked to the host viewport; ExoPlayer crops via
        // VIDEO_SCALING_MODE_SCALE_TO_FIT_WITH_CROPPING.
        configurePlaybackSurfaceBufferSize(videoSize.width, videoSize.height, "onVideoSizeChanged")
        lockPlaybackSurfaceToHostViewport("onVideoSizeChanged")
    }
    ...
    /**
     * Keep the Leanback SurfaceView sized to the fragment host (fullscreen),
     * independent of the current stream's coded width/height.
     * Intentionally does not call VideoSupportFragment.onVideoSizeChanged —
     * that path resizes the SurfaceView to the stream aspect and causes zoom
     * jumps during live ABR.
     */
    private fun lockPlaybackSurfaceToHostViewport(source: String) {
        val host = view ?: return
        val surfaceView = boundPlaybackSurfaceView ?: findSurfaceView(host) ?: run {
            Log.w(TAG, "lockPlaybackSurfaceToHostViewport: no SurfaceView source=$source")
            return
        }
        val lp = surfaceView.layoutParams ?: ViewGroup.LayoutParams(
        ...
        if (needsUpdate) {
            ...
            Log.i(
                TAG,
                "Locked SurfaceView to MATCH_PARENT host viewport " +
                    "source=$source host=${host.width}x${host.height} " +
